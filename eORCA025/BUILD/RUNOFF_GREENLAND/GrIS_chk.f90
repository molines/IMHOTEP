PROGRAM GrISchk
 ...
 ierr = NF90_INQ_DIMID(ncid,'nav_lev',id ) ; ierr = NF90_INQUIRE_DIMENSION(ncid,id, len=npk )

 ALLOCATE(dlamt(npiglo,npjglo), dphit(npiglo,npjglo), gdepw_0(npiglo, npjglo), mbathy(npiglo, npjglo) )
 ALLOCATE(rdep(npiglo,npjglo) )
 ierr = NF90_INQ_VARID(ncid,'glamt'  , id)  ; ierr = NF90_GET_VAR(ncid, id, dlamt    )
 ierr = NF90_INQ_VARID(ncid,'gphit'  , id)  ; ierr = NF90_GET_VAR(ncid, id, dphit    )
 ierr = NF90_INQ_VARID(ncid,'mbathy' , id)  ; ierr = NF90_GET_VAR(ncid, id, mbathy   )

 rdep = 0.
 DO ji=1, npk
   ierr = NF90_INQ_VARID(ncid,'gdepw_0', id)  ; ierr = NF90_GET_VAR(ncid, id, gdepw_0(:,:), start=(/1,1,ji,1/), count=(/npiglo,npjglo,1,1/)   )
   WHERE ( mbathy == ji ) rdep = gdepw_0
 ENDDO
 DO ji=1, npoints
    ii=nii(ji)  ; ij=nij(ji) 
 ...
 DO ji = 1, npoints
    ii=nii(ji)  ; ij=nij(ji)  
     IF (rdep(ii,ij) < drnfdep(ji) ) THEN
       ...
       PRINT '(i4,2f5.0,2I5,2f9.3)', ji, rdep(ii,ij), drnfdep(ji), ii,ij,dlon(ji),dlat(ji)
         DO jj1 = ij+3, ij-3, -1
