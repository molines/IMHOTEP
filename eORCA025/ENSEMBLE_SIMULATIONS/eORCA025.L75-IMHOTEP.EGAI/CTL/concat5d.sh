...
#    lst=( $( ls -1Sr  | awk -F_ '{print $NF}' | sort -u | awk -F. '{print $1}' ) )
    lst=( ICBT gridTsurf flxT icemod gridT gridU gridV )
...
if [ $ierr = 0 ] ; then
   for typ in ${lst[@]} ; do
       for mbr in ${mbrlst[@]} ; do
         ...
         wkdir=$DTADIR/$year
         cd $wkdir 
         ncrcat  -O ${CONFCASE}_y${year}m??d??.${freq}_$typ.nc  $DTADIR/${year}-concat/${CONFCASE}_y${year}_${freq}_$typ.nc &
       done
       wait
   done
   wait
