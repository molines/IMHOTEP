...
CONTAINS
   ...
   SUBROUTINE icb_trj_init( ktend )
      ...
      IF ( ln_ensemble) THEN
         cl_filename = 'trajectory_icebergs_'//cldate_ini//'-'//cldate_end//TRIM(cn_member)
      ELSE
...
#if defined key_drakkar_ensemble
     IF (ln_ensemble ) THEN
         iret = NF90_CREATE( TRIM(cn_icbdir_trj)//"/"//TRIM(cn_member(2:))//"/"//TRIM(cl_filename), NF90_CLOBBER, ntrajid )
     ELSE
