...
#if defined key_drakkar_ensemble
      REAL(wp), ALLOCATABLE, DIMENSION(:,:)   ::  ztsatem, ztsasal, zqsr
      REAL(wp), ALLOCATABLE, DIMENSION(:,:)   ::  zicefr
#endif
      !!----------------------------------------------------------------------
      !
...
! test Ice mask
         ALLOCATE (zicefr(jpi,jpj))
         CALL mpp_ens_ave_std( at_i, zicefr ) ! sea surface temperature
!!!
      ...
      !  we assume runoff on surface only
         CALL mpp_ens_ave_std( tsa(:,:,1,jp_tem), ztsatem ) ! sea surface temperature
         CALL mpp_ens_ave_std( tsa(:,:,1,jp_sal), ztsasal ) ! sea surface temperature
         CALL mpp_ens_ave_std( qsr(:,:), zqsr )             ! solar flux will be used in tra_qsr for penetrative flux

      ! Again, tsa at this stage only hold surface forcing. ( We avoid the work with another array like ztrdt, ztrds...)
         WHERE ( zicefr == 0 ) tsa(:,:,1,jp_tem) = ztsatem
         WHERE ( zicefr == 0 ) tsa(:,:,1,jp_sal) = ztsasal
         WHERE ( zicefr == 0 ) qsr(:,:) = zqsr     ! replace solar flux by ensemble mean for tra_qsr.
         CALL iom_put( "mqnet", ztsatem*rau0*rcp )
         CALL iom_put( "msfx" , ztsasal*rau0 )
         CALL iom_put( "mqsr" , zqsr )
         DEALLOCATE (ztsatem, ztsasal, zqsr, zicefr )
!}
      ENDIF
#endif
