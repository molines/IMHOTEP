...
!: Variable used in fldread to read the forcing file (nn_isf == 4 .OR. nn_isf == 3)
   ...
   INTEGER                    :: nn_rnfisf_freq     !: number of runoff data set
   TYPE(FLD_N), DIMENSION(5)  :: sn_rnfisf2         !: information about the extra runoff file to be read
   TYPE(FLD_N), DIMENSION(6)  :: slf_rnfisf         !: information about all the runoff in namelist
#endif
...
#if defined key_drakkar
               fwfisf(:,:) = 0._wp
               DO ji = 1, nn_rnfisf_freq
                 fwfisf(:,:) = fwfisf(:,:) - sf_rnfisf(ji)%fnow(:,:,1)         ! fresh water flux from the isf (fwfisf <0 mean melting) 
               ENDDO
#else
...
#if defined key_drakkar
      INTEGER           :: ierror2
      NAMELIST/namsbc_isf_drk/ nn_rnfisf_freq, sn_rnfisf2
#endif
      !!----------------------------------------------------------------------
...
#if defined key_drakkar
            IF(lwp ) WRITE(numout,*)
            IF(lwp ) WRITE(numout,*) '   ==>>>   runoffs-isf inflow read in ',nn_rnfisf_freq,' file(s)'
            ALLOCATE( sf_rnfisf(nn_rnfisf_freq), STAT=ierror )
            DO ji = 1, nn_rnfisf_freq
              ALLOCATE ( sf_rnfisf(ji)%fnow(jpi,jpj,1)  , STAT=ierror2) ; ierror = ierror + ierror2
              ALLOCATE ( sf_rnfisf(ji)%fdta(jpi,jpj,1,2), STAT=ierror2) ; ierror = ierror + ierror2
            ENDDO
            IF( ierror > 0 ) THEN
               CALL ctl_stop( 'sbc_rnf_init: unable to allocate sf_rnfisf structure' )   ;   RETURN
            ENDIF
            slf_rnfisf(1)  = sn_rnfisf
            DO ji = 2, nn_rnfisf_freq
              slf_rnfisf(ji) = sn_rnfisf2(ji-1)
            ENDDO
            CALL fld_fill( sf_rnfisf, slf_rnfisf(1:nn_rnfisf_freq), cn_dirisf, 'sbc_isf_init', 'read fresh water flux isf data', 'namsbc_isf_drk', no_print )
#else
