# IMHOTEP.S : Climatological reference
## Description
This run is identical to IMHOTEP02, except for the SSS restoring: Instead of having a restoring term, we add a correction term to the E-P, computed
from the SSS restoring term build in IMHOTEP02 (the spin-up run). We use an interannual, monthly average for this term.

### Code modification (sbcblk.F90)
A logical variable (`ln_wdmp`) defined in the namelist activates the corresponding code, if set to true.  In this case, a new forcing field is
read from the namelist (`sn_wdmp`)
...
```
Note that for consistency with the SSS restoring procedure, we also take the heat flux corresponding to this correction term, assuming that it has a temperature identical to SST.
...
!-----------------------------------------------------------------------
   ln_wdmp  = .true.          ! add FW correction to emp from previous sss damping
   !_______!________________________!___________________!____________!_____________!_________!___________!__________________!__________!_______________!
   !       !  file name             ! frequency (hours) ! variable   ! time interp.!  clim   ! 'yearly'/ ! weights filename ! rotation !  lsm          !
   !       !                        !  (if <0  months)  !   name   . !   (logical) !  (T/F)  ! 'monthly' !                  !  paring  !               !
   sn_wdmp = 'eORCA025.L75-IMHOTEP02_WDMP'  , -1.       , 'sowafld'  ,  .true.     , .false. , 'yearly'  , ''               , ''       ,  ''
...
!-----------------------------------------------------------------------
   ...
   sn_rnf      = 'eORCA025_runoff_ISBA_noAA_noGR_clim_366' , 24.       , 'sorunoff', .true.      , .true. , 'yearly'  , ''       , '' , ''
...
!-----------------------------------------------------------------------
   ...
   !           !  file name              ! frequency (hours) ! variable  ! time interp.!  clim  ! 'yearly'/ ! weights filename ! rotation ! land/sea mask !
   !           !                         !  (if <0  months)  !   name    !   (logical) !  (T/F) ! 'monthly' !                  ! pairing  !    filename   !
   sn_rnf2(1)  = 'eORCA025.L75_1m_greenland_rnfbis_clim_y1950-1972' , -1. , 'sorunoff', .true.      , .true. , 'yearly'  , ''       , '' , ''
/
```
...
!-----------------------------------------------------------------------
   ...
   !           !  file name              ! frequency (hours) ! variable  ! time interp.!  clim  ! 'yearly'/ ! weights filename ! rotation ! land/sea mask !
   !           !                         !  (if <0  months)  !   name    !   (logical) !  (T/F) ! 'monthly' !                  ! pairing  !    filename   !
   sn_icb2(1)  = 'eORCA025.L75_1m_greenland_calvingbis_clim_y1950-1972' , -1. ,'soicbclv' , .TRUE. , .true. , 'yearly'  , ''               , ''       , ''
/
