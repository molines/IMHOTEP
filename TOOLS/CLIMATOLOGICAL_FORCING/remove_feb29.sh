...
# first deal with w10 wu10 and wv10 files in WORK_DIR
cd $WORK_DIR/
...
# rename no29feb file to original name (WORK_DIR !!! )
 rename .nc.no29feb  .nc  *.no29feb

#  Then  deal with files in 3H directory
#  resulting files are put in WORK_DIR (same name !!! )
cd $DATA_FORCING/$DATA_SET/$DATA_DIR_3H
for y in $( seq $y1 $y2 ) ; do
    if [ $( isleap $y ) ] ; then
       for f in ${prefix}*_${DATA_SET}_y$y.nc ; do
          echo removing Feb 29 in $f
          cdo del29feb $f $WORK_DIR/$f 
       done
    fi
done

echo "All ${prefix}'*' files are noleap in $WORK_DIR"
