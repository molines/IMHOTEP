...
#    carte = Basemap(llcrnrlon=vp[0]-eps, llcrnrlat=vp[1]-eps, urcrnrlon=vp[2]+eps, urcrnrlat=vp[3]+eps, \
    carte = Basemap(width=5000000,height=5000000, \
                    resolution='h', area_thresh=10., projection=proj,boundinglat=40, lon_0=lon_0,lat_0=lat_0,lat_ts=lat_ts, \
                    epsg=None)
...
#   xstep=45
#   ystep=20
    carte.drawmeridians(nmp.arange(vp[0],vp[2]+xstep,xstep), labels=[0,1,0,1], linewidth=0.3)
#    carte.drawparallels(nmp.arange(vp[1],vp[3]+ystep,ystep), labels=[1,1,1,1], linewidth=0.3)
    carte.drawparallels(nmp.arange(vp[1],vp[3]+ystep,ystep), linewidth=0.3, labels=[1,0,0,0])
