...
FIGDIR=$WORK/${CONFIG}/${CONFCASE}-PLOT/PSI/PACIF
...
for f in $MWDIRY/${CONFCASE}_y${y}.${freq}_PSI.nc ; do
   ff=$(basename $f )_masked.nc
   cdfmltmask -f $f -m $WORK/$CONFIG/${CONFIG}-I/${CONFIG}_byte_mask.nc -v $var -p F -noup -o $ff
   g=${ff%.nc} 
   cf_out=${CONFCASE}_y${y}.${freq}_PSI_PACIF
...
#     (  ln -sf $f $ff
      ( python_plot.py -i $g -v $var  -p $pal -proj $proj -xstep $xstep -ystep $ystep \
            ...
            -figsz $width $height -res $res -klev $klev  -pc $charpal  -tick $tick -o $cf_out -orca
      rm ./$ff  ) &
