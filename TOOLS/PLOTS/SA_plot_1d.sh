...
MWDIR=$SDIR/${CONFIG}/${CONFCASE}-S/$freq/
...
for y in $(seq $y1 $y2 ) ; do
   MWDIRY=$MWDIR/${y}-concat

for f in $MWDIRY/${CONFCASE}_y${y}m??_${freq}_gridTsurf.nc ; do
   ff=$(basename $f )
   g=${ff%.nc} 
   tag=$( echo $g | awk -F_ '{print $2}' )
   cf_out=${CONFCASE}_${tag}_${var}_${region}
